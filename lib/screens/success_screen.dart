...
import 'package:prog2435_002_group01/main.dart';
...
String returnYesNo(bool result){ // takes a bool, returns a string
  if (result) {
    return "Yes";
  } else {
    return "No";
...
class SuccessScreen extends StatelessWidget {
  ...
  final String customerEmail; // email of customer -- cannot be blank & must be valid
  ...
  Future<void> saveData() async {
    ...
    prefs.setString("KEY_STOCK_ITEMS_LIST", stockItemsAsString); // to save the purchase
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        const Center( // center screen element
          child: Text(
            'Success!', // Header
            style: TextStyle(
              fontSize: 28,
        ...
        Container(margin: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.deepPurple, border: Border.all(color: Colors.black, width: 5)),
          child: 
        Center(
        child: Padding(
          padding: const EdgeInsets.all(16.0), // padding for layout beautification
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start, // set horizontal alignment
            mainAxisAlignment: MainAxisAlignment.start, // set vertical alignment
            children: [
              const Center( // center screen element
                child: Text(
                  'Receipt', // header
              ...
              Text(
                ...
                style: const TextStyle(fontSize: 18, color: Colors.white), // style the text
              ),
              Text(
              ...
              const Divider(height: 30, thickness: 3, color: Colors.white), // line for separation
              ...
              (isMember) ? Text("Member Discount: \$${(total * 0.05).toStringAsFixed(2)}", style: const TextStyle(fontSize: 18, color: Colors.white),) : const Text("No discount", style: TextStyle(fontSize: 18, color: Colors.white)),
              ...
              Center( // center the element on screen
                child: ElevatedButton(
                  onPressed: () { // button click event
                    Navigator.push(context, MaterialPageRoute(builder: (context) { // stack navigation
                  ...
                  style: ElevatedButton.styleFrom( // button styling
                    ...
                    padding: const EdgeInsets.symmetric(horizontal: 50, vertical: 15), // padding for layout beautification
                  ),
                  // Button label
                  child: const Text('Back to Home', style: TextStyle(color: Colors.deepPurple, fontSize: 18),),
                  ),
                ),
              ],),
            ),
